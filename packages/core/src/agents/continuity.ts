...
export interface AuditResult {
  ...
  /** The single best sentence from the chapter (dimension 42), or undefined if not found. "_none_" means no sentence worth keeping. */
...
const DIMENSION_LABELS: Record<number, { readonly zh: string; readonly en: string }> = {
  ...
  41: { zh: "设定偏离", en: "Setting Drift Check" },
  42: { zh: "一章一句", en: "Chapter Worth Check" },
};
...
function buildDimensionNote(
  ...
  if (id === 7) {
    return language === "en"
      ? "Pacing (merged dims 7/15/25/26). Check all four: (a) mini-goal cycle — do recent 5-8 chapters form a complete build-up→escalation→climax→aftermath? Allow 3-5 pure buildup chapters. 8+ chapters without climax → flag. (b) payoff dilution — did this chapter create an emotional gap or deliver a payoff exceeding 70% of built-up anticipation? Buildup/transition chapters skip this sub-check. (c) arc flatline — cross-check emotional_arcs: are any major characters stuck in one pressure shape across multiple chapters? Distinguish 'unchanged circumstances' from 'unchanged inner movement'. (d) type monotony — cross-check chapter_summaries type distribution: warn when the recent sequence stays in one mode too long. Relaxation chapters (消化章/留白章/意境章) skip this dimension entirely."
      : "节奏检查（合并维度 7/15/25/26）。同时检查四件事：(a)小目标周期——最近5-8章是否形成完整的蓄压→升级→爆发→后效？允许3-5章纯铺垫。连续8章以上无爆发→标记。(b)爽点虚化——本章是否制造了情绪缺口或完成了超出70%预期的兑现？蓄势章/过渡章自动跳过此子项。(c)弧线平坦——对照emotional_arcs：主要角色是否长时间停留在同一情绪压力形态？区分'处境未变'与'内心未变'。(d)类型单调——对照chapter_summaries类型分布：近期是否长期停留在同一模式、回收/释放章节缺席过久？松弛章（消化章/留白章/意境章）此维度整体跳过。";
  }

  if (id === 15) {
    // Merged into dimension 7. This note only appears if an older genre profile still lists 15 standalone.
    return language === "en"
      ? "Payoff dilution — now merged into dimension 7 (pacing). If both 7 and 15 are active, skip this one to avoid double-counting."
      : "爽点虚化——已合并至维度7（节奏检查）。如果7和15同时活跃，请跳过本维度避免重复。";
  }

  if (id === 25) {
    // Merged into dimension 7.
    return language === "en"
      ? "Arc flatline — now merged into dimension 7 (pacing). If both 7 and 25 are active, skip this one."
      : "弧线平坦——已合并至维度7（节奏检查）。如果7和25同时活跃，请跳过本维度。";
  }

  if (id === 26) {
    // Merged into dimension 7.
    return language === "en"
      ? "Pacing monotony — now merged into dimension 7 (pacing). If both 7 and 26 are active, skip this one."
      : "节奏单调——已合并至维度7（节奏检查）。如果7和26同时活跃，请跳过本维度。";
  }
...
description 中要明确引用 hook_id，并把状态列中 stale / blocked 的原文标记字面抄进去。本维度只审结构，不评价伏笔文笔。`;
    ...
    case 38:
      return language === "en"
        ? "Check atmosphere consistency: Do NOT judge atmosphere quality — atmosphere cannot be scored. Instead, check for elements that BREAK the established atmosphere: (1) Does a modern/urban tone intrude into a xianxia scene? (2) Does a sudden info-dump or mechanical explanation pull the reader out of the sensory environment? (3) Is there a jarring tonal shift within a single scene (e.g., solemn → comedic without transition)? A chapter with consistent mood passes, regardless of how 'good' you think the atmosphere is. Atmosphere is felt, not audited."
        : "检查氛围一致性：不要评判意境好坏——意境不可打分。只检查破坏已建立氛围的元素：(1)是否有现代/都市腔混入仙侠场景？(2)是否突然插入技术说明/信息灌输破坏了感官沉浸？(3)同一场景内是否有突兀的调性跳转（如沉静→戏谑无过渡）？氛围一致的章节即通过，不因'意境不够好'扣分。意境只能感受，不可审计。";
    case 39:
      return language === "en"
        ? "Check literary quality: Are there memorable phrases, striking metaphors, or evocative descriptions that demonstrate craft? Look for: (1) Original imagery (not clichéd comparisons), (2) Sentence rhythm variation (long/short, declarative/fragments), (3) Word choice precision (verbs over adjectives, concrete over abstract). A chapter can be technically correct yet flat — this dimension catches that."
        : "检查文采表现：是否有令人难忘的语句、精妙的比喻、或唤起共鸣的描写展现文字功力？检查：(1)原创意象（非陈词滥调的比喻），(2)句式节奏变化（长短句、陈述句/片段），(3)用词精准（动词优于形容词，具体优于抽象）。一个章节可以技术正确但平淡——这个维度捕捉这种情况。";
    case 40:
      return language === "en"
        ? "Check narrative rhythm: Does the chapter have a musical quality — moments of acceleration and deceleration, tension and release? Look for: (1) Scene transitions that feel organic, (2) Dialogue-to-narrative ratio that serves the mood, (3) Paragraph length variation that guides the reader's eye. A chapter with uniform pacing throughout, regardless of content, fails this check."
        : "检查叙事韵律：本章是否有音乐般的质感——加速与减速的交替、张力的蓄积与释放？检查：(1)场景转换是否自然流畅，(2)对话与叙述的比例是否服务于情绪，(3)段落长度变化是否引导读者视线。通篇节奏均匀的章节，无论内容如何，都未通过此检查。";
    case 41:
      return language === "en"
        ? "Check setting drift against specific anchors: (1) World Rules — do not contradict the three iron laws in story_frame.md (灵根起点不定终点, 悟性=算力/神魂=内存, 成仙查重), (2) Power System — verify consistency against the 灵根体系 table (6 tiers), (3) Character Abilities — do not exceed or understate what has been established in roles/*.md, (4) Information Boundary — characters must only act on information they possess. Cross-reference against volume_map.md for the current arc timeline. Any contradiction → critical."
        : "检查设定偏离（对照具体锚点）：(1)世界铁律 — 是否与 story_frame.md 中三条不可撼动铁律矛盾（灵根不定终点、悟性=算力/神魂=内存、成仙查重），(2)战力体系 — 是否与灵根体系表（6级）一致，(3)角色能力 — 是否超出或低于 roles/*.md 已建立的能力范围，(4)信息边界 — 角色是否只基于已掌握信息行动。参照 volume_map.md 当前剧情节点时间线。任何矛盾 → critical。";
    case 42:
      return language === "en"
        ? "Chapter worth — the zero-dimension check. Ask: if this chapter could only keep one sentence, which one? If no single sentence is worth keeping, this chapter is running on empty regardless of how clean it is. In your response, include a \"worth_line\" field: the single best sentence from the chapter (verbatim), or \"_none_\" if no sentence stands out. If \"_none_\", issue a critical-level finding — an empty-spin chapter that passed all other checks is still a failed chapter."
        : "一章一句——零维判断。问：如果本章只能留一句话，是哪句？如果没有一句话值得留，这章就是空转的。在你的输出中，追加一个「worth_line」字段：本章最值得留的一句话（原文照抄），如果没有则写「_none_」。如果是「_none_」，发出 critical 级别问题——一个通过所有其他检查但没有任何一句话值得记住的章节，依然是失败的章节。";
    default:
      return "";
  }
}
...
Relaxation chapter (松弛章) exemption: When the chapter_memo's 留白处理 (Leave-White Treatment) section declares the chapter as a 消化章/留白章/意境章 (digestion / deliberate-emptiness / atmosphere chapter), the following dimensions are AUTO-SKIPPED:
- Dim 7 (pacing) — relaxation chapters have their own breath rhythm, separate from event-cycle pacing
- Dim 15 (payoff dilution) — relaxation chapters intentionally withhold payoff
- Dim 25 (arc flatline) — emotional plateaus are valid in digestive chapters
- Dim 38 (atmosphere) — downgrade to info-only; flag only if modern/city tone intrudes into the mood
- Dim 40 (narrative rhythm) — relaxation rhythm follows mood, not event density
If the memo's 留白处理 section explicitly lists additional dimensions to skip, respect those as well. The skip is AUTOMATIC — do not produce any issue output for skipped dimensions, even at info level.
...
{
  "passed": true/false,
  "worth_line": "the single best sentence from this chapter (copied verbatim), or \"_none_\" if no sentence stands out",
  "issues": [
    {
      "severity": "critical|warning|info",
...
passed is false when any critical-severity issue exists, OR when worth_line is "_none_" (the chapter has no memorable sentence).

The chapter quality score is computed by the system — you do NOT produce it. Your only job is to find issues and extract the worth_line. Be thorough: a missed critical issue is worse than a false warning. Be specific: every issue description must quote chapter text or name exact data points.`
...
松弛章豁免：当 chapter_memo 的「留白处理」段声明本章为消化章/留白章/意境章时，以下维度自动跳过：
- 维度 7（节奏检查）—— 松弛章有自己的呼吸节律，不适用事件周期节奏
- 维度 38（氛围一致性）—— 降级为 info 仅记录；仅检查是否有现代/都市腔侵入
- 维度 40（叙事韵律）—— 松弛章韵律跟随情绪，不以事件密度衡量
如果 memo 的留白处理段额外列出了要跳过的维度，同样遵从。跳过是自动的——被跳过的维度不产生任何 issue，即使是 info 级别也不产出。
...
{
  "passed": true/false,
  "worth_line": "本章最值得留的一句话（原文照抄），没有则写 \"_none_\"",
  "issues": [
    {
      "severity": "critical|warning|info",
      "category": "审查维度名称",
...
passed 为 false 的条件：存在任何 critical 级别问题，或 worth_line 为 "_none_"（本章没有一句话值得记住）。

章节质量评分由系统公式计算，你不需要产出分数。你唯一的任务是找问题 + 提取 worth_line。宁可过度警惕（多报 warning）不要漏掉 critical。每个 issue 的描述必须引用原文片段或指明具体数据点。`;
...
${overrides}\n`;
  ...
  private tryParseAuditJson(json: string, language: PromptLanguage = "zh"): AuditResult | null {
    try {
      ...
      const worthLine = typeof parsed.worth_line === "string" ? parsed.worth_line : undefined;
      // score is now computed by the system, not the LLM — keep overallScore for backward compat
