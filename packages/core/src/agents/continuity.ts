...
function buildDimensionNote(
  ...
  if (id === 7) {
    return language === "en"
      ? "Check pacing rhythm: Do the recent 5-8 chapters form a complete mini-goal cycle (build-up → escalation → climax → aftermath)? Allow 3-5 chapters of pure buildup (accumulation phase) before flagging anything. If 8+ consecutive chapters pass without a climax (payoff/reward/reversal), flag as pacing stagnation. If the previous chapter was a climax/big reversal, does this chapter show change (relationships shifted, status changed, costs paid)? If it jumps straight to new build-up without showing impact, flag as 'post-climax impact missing'."
      : "检查节奏波形：最近 5-8 章是否形成了完整的「蓄压→升级→爆发→后效」周期？允许 3-5 章纯铺垫期（蓄势阶段），不在此范围内触发告警。如果连续 8 章以上没有爆发（兑现/回报/翻转），标记为节奏停滞。如果上一章是爆发/高潮/大反转，本章是否写出了改变？如果直接跳到新蓄压而没有展示前一波爆发的影响，标记为「高潮后影响缺失」。";
  }
  ...
  if (id === 15) {
    const base = gp.satisfactionTypes.length > 0
      ...
      : "";
    return language === "en"
      ? `${base}Check desire engine: Has the chapter created an emotional gap (reader wants release) OR delivered a payoff that exceeds expectations? A payoff that only satisfies 70% of built-up anticipation counts as diluted. If this chapter is in the aftermath phase of a mini-goal cycle, verify that consequences are shown — not just emotional reactions, but concrete changes to status, relationships, or resources. IMPORTANT: If this chapter is explicitly classified as a buildup/transition chapter (蓄势章/过渡章), skip this dimension — deliberate non-payoff is not dilution.`
      : `${base}检查欲望驱动：本章是否制造了情绪缺口（读者渴望释放）或完成了超出预期的兑现？只满足读者70%期待的兑现等于爽点虚化。如果本章处于小目标周期的后效阶段，检查是否展示了具体改变——不只是情绪反应，而是地位、关系或资源的实际变化。重要：如果本章类型明确标注为蓄势章/过渡章，此维度自动跳过——蓄势章的故意不兑现不是虚化。`;
  }
...
- 维度 15（爽点虚化）—— 松弛章刻意中止兑现
- 维度 25（弧线平坦）—— 情绪平台期在消化章中合理
